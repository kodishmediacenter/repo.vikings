...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.Liquid Lists-%s]: %s" %(addon_version, string))
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Liquid,Novos Conteudos Adicionados.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('Liquid Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('Liquid Searching Please wait',' ')
...
#                    print 'item link', item('link')
                    ...
                    for i in item('yt-dl'):
                        if not i.string == None:
                            ...
                            url.append(ytdl)
                elif len(item('ytube')) >0:
                    for i in item('ytube'):
                        if not i.string == None:
                            ytube = 'plugin://plugin.video.youtube/' + i.string + '/'
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Liquid,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(Liquid,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Liquid','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(Liquid,No IMDB match found ,7000,"+icon+")")
...
if mode==None:
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Liquid,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
