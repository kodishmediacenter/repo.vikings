...
addon = xbmcaddon.Addon(id='plugin.video.fenix.flix')
...
#                    print sepate
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                ...
                iloop+=1
            dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR firebrick][B]FENIX[/B][/COLOR] [COLOR white]flix[/COLOR] - Selecione uma das opções:', names)
            if index >= 0:
                playname=names[index]
