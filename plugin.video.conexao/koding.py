...
addonname = '[B][COLOR white]Cone[B][COLOR violet]x[B][COLOR  white]ão[/COLOR][/B] [B][COLOR violet]Tv[/COLOR][/B]'	
addon = youtube.addon
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Conexao Tv,Falha codigo de erro - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Conexao Tv,Falha ao se conectar com o servidor. - "+str(e.reason)+",10000,"+icon+")")

				
def SKindex():
    addon_log("SKindex")
    dialog = xbmcgui.Dialog()
    getData(youtube.MainBase,'')
    msg = ("https://pastebin.com/raw/ThdDE0xs") 
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Conexao Tv,Novos conteudos adicionados.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('Conexao Tv Seelecione uma fonte', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
def SearchChannels():
    ...
    List=[]
    List.append(youtube.MainBase)
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
    progress = xbmcgui.DialogProgress()
    progress.create('Conexao Tv Procurando espere',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Conexao Tv,Habilite as atualizações do Commonresolvers para continuar. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(Conexao Tv,Usando URL Resolver.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Conexao Tv','Escolha um local para salvar os arquivos.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Conexao Tv', 'Você deseja adicionar esse arquivo como fonte?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(Conexao Tv,Nada encontrado no IMDB ,7000,"+icon+")")
...
#        print 'parentalblockedpin',parentalblockedpin
        if len(parentalblockedpin)>0:
            if parentalblock:
                contextMenu.append(('Desabilitar controle dos pais','XBMC.RunPlugin(%s?mode=55&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
            else:
                contextMenu.append(('Habilitar controle dos pais','XBMC.RunPlugin(%s?mode=56&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
...
#                print 'setting as folder in link'
            contextMenu.append(('[COLOR white]!!Fazer download do conteudo Atual!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=21&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                ...
                contextMenu.append(('Adicionar aos Favoritos','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#        print 'Not setting setResolvedUrl'
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(Conexao Tv ,Falha ao extrair regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Conexao Tv,Por favor [COLOR yellow]instale o add-on YouTube[/COLOR] para continuar ,10000,"")")
