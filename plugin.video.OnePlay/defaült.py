...
g_ignoreSetResolved=['plugin.video.oneplay','plugin.video.f4mTester','plugin.video.shahidmbcnet','plugin.video.SportsDevil','plugin.stream.vaughnlive.tv','plugin.video.ZemTV-shani']
    ...
    for other,channel_name,stream_url in match:
        ...
        if 'type' in other:
            ...
            if mode_type == 'yt-dl':
                ...
                stream_url = 'plugin://plugin.video.oneplay/?name='+urllib.quote(channel_name) +'&url=' +stream_url +'&mode=125&ch_fanart=na'
            ...
            try:
                ...
                if len(item('link')) >0:
                    ...
                    for i in item('sportsdevil'):
                        if not i.string == None:
                            sportsdevil = 'plugin://plugin.video.oneplay/?mode=1&amp;item=catcher%3dstreams%26url=' +i.string
                    ...
                    for i in item('ilive'):
                        if not i.string == None:
                            if not 'http' in i.string:
                                ilive = 'plugin://plugin.video.oneplay/?url=http://www.streamlive.to/view/'+i.string+'&amp;link=99&amp;mode=iLivePlay'
                            else:
...
def getUrl(url, cookieJar=None,post=None, timeout=20, headers=None):
    ...
    req.add_header('User-Agent','Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, lbrlive Gecko) Chrome/33.0.1750.154 Safari/537.36')
    if headers:
        for h,hv in headers:
            req.add_header(h,hv)
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(brlive,Urlresolver donot support this domain. - ,5000)")
    return resolver
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from brlive Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
