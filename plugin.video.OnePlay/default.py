...
addon = xbmcaddon.Addon(id='plugin.video.OnePlay')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.OnePlay Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(OnePlay,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(OnePlay,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(OnePlay,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('[COLOR aquamarine][B]OnePlay[/COLOR] HD[/B] Selecione uma opçăo:', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
             try:
...
#hakamac code
    ...
    progress.create('OnePlay Searching Please wait',' ')
...
host = "=s1mLkhWehxGcl52buc3d39yL6MHc0RHa"
tam = len(host)
basedem = host[::-1]
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(OnePlay,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(OnePlay,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
#                    print sepate
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                ...
                iloop+=1
            dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR aquamarine][B]OnePlay[/COLOR] HD[/B] Selecione uma opçăo:', names)
            if index >= 0:
                playname=names[index]
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('OnePlay','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('OnePlay', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(OnePlay,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false":
                ...
                name = name + '[COLOR aquamarine] (' + str(len(playlist)) + ' Opções)[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(OnePlay ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(OnePlay,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
