...
def setup_library(library_folder):
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = _("Would you like to automatically set [COLOR lightseagreen]M[/COLOR]etalli[COLOR lightseagreen]Q[/COLOR] 4[COLOR lightseagreen]Q[/COLOR]ed as a music source?")
        if dialogs.yesno("{0} {1}".format(_("Library"), "setup"), msg):
            source_thumbnail = get_icon_path("musicvideos")
            source_name = "[COLOR lightseagreen]M[/COLOR]etalli[COLOR lightseagreen]Q[/COLOR] 4[COLOR lightseagreen]Q[/COLOR]ed "  + _("Music videos")
