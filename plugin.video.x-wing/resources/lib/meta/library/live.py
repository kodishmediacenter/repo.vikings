...
def setup_library(library_folder):
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = _("Would you like to automatically set [COLOR lightseagreen]M[/COLOR]etalli[COLOR lightseagreen]K[/COLOR] D[COLOR lightseagreen]S[/COLOR]ed as a channel video source?")
...
def auto_live_setup(library_folder):
    ...
    if not xbmcvfs.exists(library_folder):
        try:
            xbmcvfs.mkdir(library_folder)
            source_thumbnail = get_icon_path("live")
            source_name = "[COLOR lightseagreen]M[/COLOR]etalli[COLOR lightseagreen]K[/COLOR] 4[COLOR lightseagreen]DS[/COLOR]ed " + _("Channels")
            source_content = "('{0}','','','',0,0,'<settings></settings>',0,0,NULL,NULL)".format(library_folder)
            add_source(source_name, library_folder, source_content, source_thumbnail)
            return True
        except:
