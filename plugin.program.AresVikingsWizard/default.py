...
def UNZIPROM():
	...
	DP.create(ADDONTITLE,'[COLOR %s][B]Baixando:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, display),'', 'Aguarde...')
...
def skinInstaller(name, url):
	if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]Addon Installer[/COLOR]" % COLOR1, '[COLOR %s]%s:[/COLOR] [COLOR %s]Invalid Zip Url![/COLOR]' % (COLOR1, name, COLOR2)); return
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	DP.create(ADDONTITLE,'[COLOR %s][B]Baixando:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), '', '[COLOR %s]Please Wait[/COLOR]' % COLOR2)
	urlsplits = url.split('/')
	lib = xbmc.makeLegalFilename(os.path.join(PACKAGES, urlsplits[-1]))
	try: os.remove(lib)
	except: pass
...
def systemInfo():
	...
	addFile('[COLOR %s]Armazenamento usado:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, storage_free), '', icon=ICONMAINT, themeit=THEME2)
...
def FavsMenu():
	...
	last = str(FAVSsave) if not FAVSsave == '' else 'Seus favoritos ainda nao foram salvos.'
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to apply the guifix for:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]Nao, Cancelar![/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]WARNING!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] community build is not currently installed." % (COLOR2, COLOR1, name), "Would you like to apply the guiFix anyways?.[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar![/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		if yes:
			...
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Para que a instalacao seja efetuada de modo correto, sera necessario o fechamento forcado do KODI, por favor, pressione em OK, para encerrar e abra novamente o Kodi[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
		...
		if yes_pressed:
			...
			DP.update(0, title,'', 'Por favor, agaurde...')
			percent, errors, error = extract.all(lib,HOME,DP, title=title)
			if int(float(percent)) > 0:
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Para que a instalacao seja efetuada com sucesso, sera necessario encerrar seu Kodi, por favor, clique em OK para finalizar.[/COLOR]" % COLOR2); wiz.killxbmc('true')
			else:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]A Vikings Build [COLOR %s]%s[/COLOR] possibilita a instalacao de [COLOR %s]%s[/COLOR] Tema adulto!" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Voce pode efetuar a instalacao nesse momento, clicando em INSTALAR ADULTO, ou efetuar a instalao posteriormente? Se desejar instalar o conteudo adulto, clique em INSTALAR ADULTO, ou CANCELAR para prosseguir![/COLOR]", yeslabel="[B][COLOR green]INSTALAR ADULTOS[/COLOR][/B]", nolabel="[B][COLOR red]CANCELAR[/COLOR][/B]"):
...
def addonpackwiz():
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to install the:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B]Nao, Cancelar![/B]',yeslabel='[B]Install[/B]')
		if yes:
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Please Wait')
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Clique em [B][COLOR springgreen] - Sim - [/COLOR][/B]" % COLOR2, "Para iniciar a instalacao, \r\nThen Install a Local or External Stored Build Back Up[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar![/COLOR][/B]', yeslabel='[B][COLOR springgreen]SIM[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Clique em [B][COLOR springgreen] - SIM - [/COLOR][/B]" % COLOR2, "Para iniciar uma nova instalacao da Build: , \r\nSera feita uma nova instalacao para a Build [COLOR %s]%s[/COLOR]!!" % (COLOR1, install), nolabel='[B][COLOR red]Nao, Cancelar![/COLOR][/B]', yeslabel='[B][COLOR springgreen]SIM[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce deseja restaurar seu KODI" % COLOR2, "para as configruaoes padroes?[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar![/COLOR][/B]', yeslabel='[B][COLOR springgreen]SIM[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
		...
		DP.update(0, "[COLOR %s]Limpando os arquivos e Pastas:" % COLOR2)
...
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce Gostaria de limpar o Cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Sim, Limpar![/COLOR][/B]'):
		wiz.clearCache()
		DC.setLabel('Tamanho: [B][COLOR lime]0.0 B[/B][/COLOR]')
def clearArchive():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de limpar o \'Archive_Cache\' da pasta?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancelar![/COLOR][/B]', yeslabel='[B][COLOR green]Sim, Limpar[/COLOR][/B]'):
		wiz.clearArchive()
def clearPackages():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de deletar os pacotes?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancelar![/COLOR][/B]', yeslabel='[B][COLOR green]Sim,Delete[/COLOR][/B]'):
		wiz.clearPackages('total')
		DPK.setLabel('Tamanho: [B][COLOR lime]0.0 B[/B][/COLOR]')
		TPK.setLabel('Arquivos: [B][COLOR lime]0[/B][/COLOR]')

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce deseja limpar o cache, pacotes e imagens temporarias?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancele![/COLOR][/B]',yeslabel='[B][COLOR green]Sim, limpe tudo![/COLOR][/B]'):
		wiz.clearCache()
		wiz.clearPackages('total')
		clearThumb('total')
...
def clearThumb(type=None):
	...
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce deseja deletar o %s e as pastas de imagens temporarias?' % (COLOR2, latest), "Essas imagens sera criadas na proxima abertura do Kodi[/COLOR]", nolabel='[B][COLOR red]Nao,Cancelar![/COLOR][/B]', yeslabel='[B][COLOR green]Sim, Delete![/COLOR][/B]')
	if choice == 1:
...
def BuildList():
	...
	if not BUILDFILE == 'http://' and not BUILDFILE == '':
		...
		window.placeControl(InstallButton,28 , 20, 8, 9)
		...
		ThemeButton = pyxbmct.Button('[COLOR %s][B]Intalar Menu Adulto[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(ThemeButton,44 , 20, 8, 10)
		...
		buildname  = pyxbmct.Label('[COLOR %s]Build Instalada: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildname))
		...
		skinname  = pyxbmct.Label('[COLOR %s]Skin da Build Instalada: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,defaultskinname))
		window.placeControl(skinname, 100, 2, 11, 20)
		
		buildtheme  = pyxbmct.Label('[COLOR %s]Menu Adulto Instalado: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildtheme))
...
def Un_Hide_Net():
	...
	ISP.setLabel('[COLOR %s]Provedor de Internet:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,isp))
...
def Maint():
	...
	sysinfo_title =  pyxbmct.Label('[COLOR %s][B]Informacoes do Dispositivo[/B][/COLOR]' % DES_T_COLOR)
	window.placeControl(sysinfo_title, 12, 31, 10, 15)
	version1 =  pyxbmct.Label('[COLOR %s]Versao:[/COLOR] [COLOR %s]%s[/COLOR] - [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, codename, DESCOLOR, version))
	window.placeControl(version1, 18, 37, 10, 15)
	store = pyxbmct.Label('[B][COLOR %s]Armazenamento[/COLOR][/B]'% DESCOLOR)
	window.placeControl(store, 23, 39, 10, 10)
	rom_used=pyxbmct.Label('[COLOR %s]Usado:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, storage_free))
	window.placeControl(rom_used, 28, 39, 10, 10)
	rom_free=pyxbmct.Label('[COLOR %s]Livre:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, storage_used))
	...
	### Hello, how are you
	ram_used=pyxbmct.Label('[COLOR %s]Usada:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, ram_used))
	window.placeControl(ram_used, 48, 39, 10, 10)
	ram_free=pyxbmct.Label('[COLOR %s]Livre:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, ram_free))
	...
	kodi = pyxbmct.Label('[COLOR %s]Dispositivo:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, name))
	window.placeControl(kodi, 17, 22, 10, 15)
	totalcount = len(picture) + len(music) + len(video) + len(programs) + len(scripts) + len(skins) + len(repos) 
	total = pyxbmct.Label('[COLOR %s]Addons Instalados: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, totalcount))
	...
	netinfo_title =  pyxbmct.Label('[COLOR %s][B]Informacoes de Internet[/B][/COLOR]'% DES_T_COLOR)
	...
	ISP = pyxbmct.Label('[COLOR %s]Provedor de Internet:[/COLOR] [COLOR %s]Oculto[/COLOR]' % (DES_T_COLOR, DESCOLOR))
	...
	### I'm good, u?
	total_cache_button = pyxbmct.Button('[COLOR %s]Cache Deletado[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_cache_button, 87 , 21 ,  9, 9)
	window.connect(total_cache_button,lambda: clearCache())
	
	total_packages_button = pyxbmct.Button('[COLOR %s]Pacotes Deletado[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_packages_button, 87 , 31 ,  9, 9)
	window.connect(total_packages_button,lambda: clearPackages())
	
	total_thumbnails_button = pyxbmct.Button('[COLOR %s]Deletar Thumbnails[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	window.placeControl(speedtest_button, 58 , 1 ,  9, 7)
	window.connect(speedtest_button, lambda: runspeedtest())
	
	un_hide_net = pyxbmct.Button('[COLOR %s]Informacoes da Internet[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(un_hide_net, 58, 8, 9, 11)
	window.connect(un_hide_net, lambda: Un_Hide_Net())
	
	trigger_title =  pyxbmct.Label('[COLOR %s]Alterar as configuracoes[/COLOR]'% DES_T_COLOR)
...
def BackRes():
	...
	backupaddonpack_button = pyxbmct.Button('[COLOR %s] Pack de Addons[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	backup_addondata_button = pyxbmct.Button('[COLOR %s] Addons de Dados[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
InstallablesButton = pyxbmct.Button('[COLOR %s][B]Instalacoes[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
