...
AddonID = 'plugin.video.guiadoluiz'
...
addon_data_dir = xbmc.translatePath("special://home/addons/plugin.video.guiadoluiz/data/").decode("utf-8")
...
def Categories():
	...
	for item in list:
		...
		AddDir("[COLOR yellow]{0}[/COLOR]".format(name) ,item["url"].encode("utf-8"), mode, image.encode("utf-8"), logos.encode("utf-8"), index=i, cacheMin=cacheMin)
		i += 1
