...
# Conteudo = 'http://www.mmfilmes.tv/ultimos/'
#ip:2374.1247.01.01
...
def makeRequest(url, headers=None):
        try:
            if headers is None:
                headers = {'User-agent' : 'plugin.video.atto('+__Header__+') /kodi.addons/16.1/17/18'}
...
def callbits(url,fanart):
    ...
    if isinstance(soup,BeautifulSOAP):
        ...
        if len(soup('channels')) > 0:
            ...
            if Sy50p3r_C4ch == Sy50p_PIXC_C4ch :
                msgsuccessful()
                getItems(soup('item'),FANART)
                Dlss=1 

            if Sy50p3r_C4ch == Sy50p_PIXN_C4ch :
                msgsuccessful()
                getItems(soup('item'),FANART)   
                Dlss=1
...
def IIIiD_temp():
    msg="W0NPTE9SIGdvbGRdUGVzcXVpc2FuZG8gbGlua3MgZGEgaW50ZXJuZXQuLi5bL0NPTE9SXQ=="
    msg= base64.b64decode(msg)
    xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%('...',msg, time, icon))
