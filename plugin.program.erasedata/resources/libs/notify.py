...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "Existe um pacote de atualizacao e correcao para a BUILD:\n[COLOR %s]%s[/COLOR]\n\nAviso Importante: Detectamos que atualmente esta instalado neste dispositivo a versao: [COLOR %s]%s[/COLOR]\nRecomendamos que voce atualize e aplique o pacote de atualizacao para perfeito funcionamento, apos a atualizacao, a sua versao de build istalada, passara a ser: [COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Para atualizar, pressione a opcao Instalacao Normal[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
