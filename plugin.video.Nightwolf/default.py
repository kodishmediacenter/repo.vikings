...
addon = xbmcaddon.Addon(id='plugin.video.Nightwolf')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.nightwolf Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(nightwolf,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(nightwolf,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")


off1  =  "==wchlmcvdWZ0F2QvIXZ0NXYt9iZs92d0h2Zp52Lu9GZkFmZs92d0h2Zp52Lt92YuQnblRnbvNmclNXdiVHa0l2ZucXYy9yL6MHc0RHa"
...
play = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.nightwolf/playlistv.xml").decode("utf-8"))
...
def SKindex():
    ...
    getData(off,'')
    msg2 = "https://pastebin.com/"	#COLOQUE O LINK DA MENSAGEM DO ADD-ON AQUI NO LUGAR DO LINK DO PASTEBIN OBS: APAGUE O JOGO DA VELHA DE #xbmc.executebuiltin PARA ATIVAR ESSA OPÇÃO!		
    msg = msg2 
...
def addlista():
		...
		link = dialog.input('[COLOR yellow]Cole sua Lista M3U ou XML[/COLOR]','https://pastebin.com', type=xbmcgui.INPUT_ALPHANUM)
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(nightwolf,New source added.,5000,"+icon+")")
        if not url is None:
...
#hakamac code
    ...
    progress.create('nightwolf Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(nightwolf,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(nightwolf,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
#                    print sepate
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                ...
                iloop+=1
            dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR white][B]Nightwolf[/B][/COLOR] === Selecione Uma Link ===', names)
            if index >= 0:
                playname=names[index]
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('nightwolf','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('nightwolf', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(nightwolf,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if regexs: 
            ...
            if addon.getSetting('dlaudioonly') == 'true':
                ...
                url = 'plugin://plugin.video.f4mTester/?url=' +url+ '&amp;streamtype=HLS&amp;name=nightwolf'
                mode = '12'
        elif '.ts' in url:
                url = 'plugin://plugin.video.f4mTester/?url=' +url+ '&amp;streamtype=TSDOWNLOADER&amp;name=nightwolf'
                mode = '12'
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false":
                ...
                name = name + '[COLOR dimgreen] (' + str(len(playlist)) + ' items )[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
...
#        print 'Not setting setResolvedUrl'
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(nightwolf ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(nightwolf,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
