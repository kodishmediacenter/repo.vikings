...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(keysey,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(keysey,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('keysey','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(keysey,No IMDB match found ,7000,"+icon+")")
...
if mode==None:
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(keysey,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
