...
mysettings = xbmcaddon.Addon(id = 'plugin.video.GATODOGRUPO')
...
def main():
	add_dir('[COLOR white] CLIQUE ABAIXO! [B][/COLOR][/B]', 'searchlink', 99, icon, fanart)
	if len(online_m3u) > 0:	
		add_dir('[COLOR yellow][B] CASO FIQUE OFF, COLOQUE OUTRA LISTA NAS CONFIGURAÇÕES. [/B][/COLOR]', u_tube, 2, icon, fanart)
	if len(local_m3u) > 0:	
		add_dir('[COLOR magenta][B]>> LOCAL M3U <<[/B][/COLOR]', u_tube, 3, icon, fanart)
	if len(online_xml) > 0:	
		add_dir('[COLOR white][B] LOCAL XML[/COLOR][/B]', u_tube, 4, icon, fanart)
	if len(local_xml) > 0:	
