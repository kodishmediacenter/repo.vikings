...
icon = os.path.join(xbmc.translatePath("special://home/addons/quasar.repo.store/icon.png").decode("utf-8"))
...
base='kodi.tv'
ADDON=xbmcaddon.Addon(id='quasar.repo.store')
...
def KRATOSVIP():
    link = OPEN_URL('https://raw.githubusercontent.com/quasarmediacenter/store/master/kratosvip').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def CATEGORIES2():
    link = OPEN_URL('https://raw.githubusercontent.com/quasarmediacenter/store/master/quasarstoreraptor2').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def Reposquasar():
    link = OPEN_URL('https://raw.githubusercontent.com/quasarmediacenter/store/master/repos-quasarstore').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def RESTRITOM18():
    link = OPEN_URL('https://raw.githubusercontent.com/quasarmediacenter/store/master/so18').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def TVADDONS():
    link = OPEN_URL('https://raw.githubusercontent.com/quasarmediacenter/store/master/tvaadons').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def ATIVAR():
    link = OPEN_URL('https://raw.githubusercontent.com/quasarmediacenter/store/master/ative').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def PREMIUM():
    link = OPEN_URL('https://raw.githubusercontent.com/quasarmediacenter/store/master/premiun').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def Traducao():
    link = OPEN_URL('https://raw.githubusercontent.com/quasarmediacenter/store/master/04062018/traducao').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
...
def EXTZSETUP():
        ...
        keylink.doModal()
        ulink = keylink.getText()
        link = OPEN_URL("https://raw.githubusercontent.com/quasarmediacenter/store/master/zip").replace('\n','').replace('\r','')
        match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
        name = "Zip Instaler"
        #url  = ""+ulink+""
        iconimage = ""
...
def menuquasar():
       dialog = xbmcgui.Dialog()
       ret = dialog.select('[COLOR yellow]Bem Vindo ao Quasar Instaler / Welcome Quasar Instaler[/COLOR]', ['Quasar'])
