...
import time
import requests
import disclaimerforpoliceonly

__addon__ = xbmcaddon.Addon()
__addonname__ = __addon__.getAddonInfo('name')
__icon__ = __addon__.getAddonInfo('icon')
 
line1 = "O Melhor do Brasil para os Melhores Brasileiros"
...
addon = disclaimerforpoliceonly.addon
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(BrasilFull,Ops algo de errado aconteceu - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(BrasilFull,Erro servidor está offline. - "+str(e.reason)+",10000,"+icon+")")

				
def SKindex():
    addon_log("SKindex")
    addDir('Favorites','Favorites',4,'http://goo.gl/TyDD6w' ,  FANART,'','','','')
    getData(disclaimerforpoliceonly.MainBase,'')
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('BrasilFull Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    List=[]
    List.append(disclaimerforpoliceonly.MainBase)
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
    progress = xbmcgui.DialogProgress()
    progress.create('BrasilFull Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(BrasilFull,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(BrasilFull,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        ...
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('BrasilFull', 'Do you want to add this file as a source?')
        if ret:
