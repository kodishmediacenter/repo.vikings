...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(BrasilFull,Falha ao se conectar com o servidor. - "+str(e.reason)+",10000,"+icon+")")
...
def SKindex():
    ...
    msg = ("https://pastebin.com/raw/3xSMUxVC") 
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(BrasilFull,Novos conteudos adicionados.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('BrasilFull Seelecione uma fonte', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(BrasilFull,Habilite as atualizações do Commonresolvers para continuar. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(BrasilFull,Usando URL Resolver.. - ,5000)")
        import urlresolver
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(BrasilFull,Nada encontrado no IMDB ,7000,"+icon+")")
...
#        print 'Not setting setResolvedUrl'
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(BrasilFull ,Falha ao extrair regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(BrasilFull,Por favor [COLOR yellow]instale o add-on YouTube[/COLOR] para continuar ,10000,"")")
